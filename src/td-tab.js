...
export function initTdTab(root) {
  ...
  function makeSectorRun() {
    return createSectorRun({ story, host: root, api: storyApi, waveSize: params.waveSize, threatMult, hardcore: story.hardcore, spawnGap: { spread: SPAWN_SPREAD, max: SPAWN_GAP_MAX }, store: localStorage, rng: () => whim(), now: () => t,
      ...
      field: () => ({ cellSide, centers: graph.centers, dist: bfsDist(graph.adj, [dungeon.heart], (ci) => dungeon.tags[ci] !== BLOCKED), inside: (ci) => story.inside(ci), rim: (() => { const seeds = []; for (let ci = 0; ci < graph.centers.length; ci++) if (story.inside(ci)) seeds.push(ci); return bfsDist(graph.adj, seeds, () => true); })(),   /* hops from the clearing over any ground, rock or not: a breach's blast carves rock */ excluded: [...sealedBreachCells, ...spawnPoints.filter((s) => s.alive).map((s) => s.ci)], farHops: Math.round(60 * 1.15), fallback: () => gunshipFar() }),   /* the ring gunshipFar walks: a minute out */
      open: (ci, o) => storyApi.breach(ci, o), collapse: (sp) => killPortal(sp, 'exhausted'), queued: (sp) => spawnQueue.some((q) => q.sp === sp), spReady: (sp) => gameBreaches.ready(sp.obj),
  ...
  const gameHooks = {
      ...
      sectorRelease: (id) => sectorRun?.test.release(id) ?? null, sectorClose: (id, by) => sectorRun?.test.close(id, by) ?? null, sectorClearField: () => sectorRun?.test.clearField(), sectorCull: (r = 8) => { if (!story) return 0; const doors = [story.gateCell, ...(story.backOpen ? story.backMouth?.cells ?? [] : [])].filter((c) => c >= 0).map((c) => graph.centers[c]); let n = 0; for (const e of enemies) if (e.alive && !e.guard && e.emergeAge >= 1.2 && doors.some((c) => chord(e.pos, c) < cellSide * r)) { killCreature(e, false); n++; } return n; },   /* THE PACING PROBE'S DEFENDER (--pacing): every sector body that reaches a door within r cells dies there, so the arrivals it counts are the design's rhythm, not a player's */ sectorContinue: () => sectorRun?.test.cont(), sectorKeepHolding: () => sectorRun?.test.keepHolding(), sectorQuiet: (on) => sectorRun?.test.quiet(on), sectorReport: () => sectorRun?.test.report() ?? null,
