...
export function initTdTab(root) {
  ...
  function playerHit(killerType = null, fromPos = null) {
    ...
    playerHP--; if (story?.expeditions) hullLost(story.expeditions);
    run.hullsLost++;
    checkAchievements();
  ...
  function finishOrder(order) {
    dropSiteRing(order);
    if (order.kind === 'structure') { storyApi.printed(order.step); sfx.play('tower_upgrade'); } else if (order.kind === 'tower') {   /* a piece of the base Isao printed (src/content/base-programme.js) */
      if (order.ghost) { scene.remove(order.ghost); disposeObj(order.ghost); order.ghost = null; }
  ...
  function frame(dt, simSkip) {
    ...
    if (!player.won && !frozen) {
      ...
      { const held = automated() && !onStation(gunship), ev = held ? null : stepGunship(gunship, dt, GUNSHIP_ORBIT); if (automated() && ev === 'depart') passEnded(gunshipCall, GUNSHIP_CALL); if (held) storyViews?.meter(callProgress(gunshipCall), callFull(gunshipCall)); else storyViews?.station(onStation(gunship), phaseLeft(gunship)); if (story) (gunshipOptic ??= createGunshipOptic(scene, { cellSide, metresPerCell: GUNSHIP_PLATFORM.metresPerCell })).ride(graph.centers[dungeon.heart], graph.normals[dungeon.heart], passProgress(gunship, GUNSHIP_ORBIT), onStation(gunship), graph.centers[gunshipLane()]); if (ev === 'depart') gunshipWalls = null; pilot?.gunshipTick?.(dt); } if (stepStrike(strike, dt, strikeTune) === 'armed') {   // the gunship's pass, then the strike's ration
        sfx.play('tower_upgrade');
  ...
  if (urlParams.get('acceptance') === '1') {
    window.__stalheartTest = {
      ...
      siteCells: () => story?.siteCells ?? {}, placeTank: (ci) => { player.freeMode = false; player.virtualStart = null; player.cur = ci; player.prev = ci; player.next = ci; player.prog = 0; player.segLen = cellSide; player.pos = graph.centers[ci].slice(); },   /* the same fields deployStart resets, so the step rebuilds pos ON ci instead of gliding off it; segLen is the cell scale because cur === next is a zero-length chord */ killGuards: (id) => { for (const e of enemies) if (e.alive && e.guard?.site === id) killCreature(e, false); }, hitTank: () => { if (playerHP > 1) playerHit(); }, commitTower: (key, ci) => !!commitTower(key, ci, 0), breachNew:()=>seedPortals(1),
