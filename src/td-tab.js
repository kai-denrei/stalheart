...
export function initTdTab(root) {
  ...
  const strike = makeStrike(), gunship = makeGunship(GUNSHIP_ORBIT, { station: new URLSearchParams(location.search).get('gunship') === 'station' }); let gunshipOptic = null, gunshipWalls = null; const gunshipLane = () => story?.source?.alive ? story.source.ci : story?.ring?.size ? [...story.ring][0] : dungeon.spawn;   // where they come from: the live breach, else the lane outside the gate, else the lane end. The platform's schedule runs on the game clock beside the strike's ration; ?gunship=station opens its first pass at once, for acceptance
  ...
  if (urlParams.get('acceptance') === '1') {
    window.__stalheartTest = {
      state: () => ({ gunship: { phase: gunship.phase, left: +gunship.left.toFixed(2), mounted: gunship.mounted, gun: gunship.gun, passes: gunship.passes, optic: !!gunshipOptic?.active(), station: onStation(gunship), seat: !!pilot?.gunship, lane: pilotHost?.gunship?.lane() ?? -1, aim: pilot?.gunshipOptic?.()?.pos ?? null, aimCell: pilot?.gunshipOptic?.()?.pos ? cellIndex(norm3(pilot.gunshipOptic().pos)) : -1 }, shot:shotId(),breachRubble:gameBreaches.rubbleState(),breaches:gameBreaches.state(),queued:spawnQueue.length,wallCount:dungeon.tags.filter(tag=>tag===BLOCKED).length,emerging:enemies.filter(e=>e.alive&&e.emergeAge<1.2).length,round, wave, runGen: runContext.generation, heart: heartHP, hulls: playerHP,
