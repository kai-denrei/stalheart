...
export function initTdTab(root) {
  ...
  const sealedBreachCells = new Set(); const strike = makeStrike(), gunship = makeGunship(GUNSHIP_ORBIT, { station: new URLSearchParams(location.search).get('gunship') === 'station' }); let gunshipOptic = null, gunshipTrack = null, gunshipWalls = null, gunshipBriefing = null, foundryFx = null, gunshipDrop = null; const gunshipFar = () => { const d = bfsDist(graph.adj, [dungeon.heart], (ci) => dungeon.tags[ci] !== BLOCKED), want = Math.min(Math.round(60 * 1.15), d.reduce((m, v) => Math.max(m, v), 0)); let best = -1, bd = Infinity; for (let i = 0; i < d.length; i++) if (d[i] >= want - 3 && d[i] <= want + 3 && ![...sealedBreachCells].some((s) => chord(graph.centers[i], graph.centers[s]) < cellSide * 6)) { const c = chord(graph.centers[i], graph.centers[dungeon.spawn]); if (c < bd) { bd = c; best = i; } } return best >= 0 ? best : dungeon.spawn; }; /* a breach a minute's walk out (owner: leave time to enjoy the action): the open cell about 69 hops from the heart nearest the lane end, the other side of the map if the planet is smaller */ const gunshipLane = () => story?.source?.alive ? story.source.ci : story?.ring?.size ? [...story.ring][0] : dungeon.spawn;   // where they come from: the live breach, else the lane outside the gate, else the lane end. The platform's schedule runs on the game clock beside the strike's ration; ?gunship=station opens its first pass at once, for acceptance
  ...
  let heartSprite = null, playerMesh = null, markerMesh = null, storyBase = null, story = null, sectorRun = null, skipCard = null, showcase = null;   /* skipCard: the SKIP TUTORIAL offer (src/fx/skip-tutorial.js) */
  ...
  function spawnIsao() {
    ...
    return preloadFabricator().then((ok) => {
      ...
      isao = { obj, dir, state: 'idle', t: 0, dur: 0, order: null, loiter: dir.slice(), gleeT: 0, assistAt: story?.assistAt ?? null }; if (story) story.assistAt = null;   /* a foundry that deployed before he landed is still his to tend */
      placeIsao();
      scene.add(obj);
      return true;
    });
  ...
  function makeSiteRing(ci, color) { return siteRing(scene, graph.centers[ci], graph.normals[ci], cellSide, params.wallHeight, color); }   // src/fx/site-ring.js
  function dropSiteRing(order) { if (!order.ring) return; disposeSiteRing(scene, order.ring); order.ring = null; }
  ...
  function orderTower(key, ci, { quiet = false } = {}) {
    const def = TOWER_BY_KEY[key];
    if (!def) return false; if (automated() && !unlockedTowers(story.expeditions, STORY_EXPEDITIONS.base).includes(key)) { flashShopNote('PART NOT HOME'); return false; }   /* the part for this wall is still out at a landing site: the same shop note the other refusals use */
    const err = placeError(ci);
    if (err) { flashShopNote(err); return false; }
  ...
  function stepWorker(w, dt) {
    ...
    if (w.state === 'idle') {
      ...
      if (free) {
        ...
        const hd = norm3(w.assistAt ?? graph.centers[dungeon.heart]);   // or the foundry he is tending on landing
  ...
  function updateIsao(dt) {
    ...
    if (working) {
      ...
      const bed = isao.order.bed ? (uu) => isao.order.bed(...printOffset(pattern, uu), isao.t / isao.dur) : (uu) => {   /* a structure's bed is its whole plot at the rising print height */
  ...
  const towerSeekers = [];
  let missilePool=null, missilesDisposed=false, talonPool=null; const missileOf = (key) => story?.missiles?.[key] ?? CONTENT.missiles[key];   // the story's Quiver fires the TALON
  ...
  function stepTowers(dt, tNow) {
    ...
    for (const tw of towers) {
      ...
      if (manual) {
        ...
        /* THE SCOPE (a guided mount in the story): the workshop's reticle and TRACK panel, fed from the same lock the launcher fires on */ if (storyScope) { const cfg = missileOf(tw.key), t = tw.pilotTarget && !tw.pilotTarget.pilotAim ? tw.pilotTarget : null, bearing = t ? Math.acos(Math.max(-1, Math.min(1, dot3(towerBarrel(tw, norm3(graph.centers[tw.ci])), norm3(sub3(t.pos, graph.centers[tw.ci])))))) * 180 / Math.PI : 0, sp = t ? scopeV.fromArray(add3(t.pos, scale3(norm3(t.pos), cellSide * 0.3))).project(camera) : null, onGlass = !!sp && sp.z < 1, lockedNow = !!tw.lock?.locked; if (lockedNow !== !!tw.lockTick) { tw.lockTick = lockedNow; if (lockedNow) sfx.play('laser_click'); } storyScope.update({ on: !!cfg && !pilot.isMap(), w: innerWidth, h: innerHeight, meter: tw.lock?.meter ?? 0, locked: lockedNow, target: t ? { id: t.id, range: distance ?? 0, bearing, type: t.type } : null, max: cfg?.maxRange ?? maxRange, zoom: pilot.state.zoom, box: innerHeight / 2 * (story?.quiverCone ?? 0) / Math.tan(camera.fov * Math.PI / 360), cone: Math.atan(story?.quiverCone ?? 0) * 180 / Math.PI, lockTime: cfg?.lockTime ?? 0, flight: towerSeekers.reduce((n, m) => n + (m.by === tw ? 1 : 0), 0), ready: !tw.obj.userData.loading && !tw.overheated, cooldown: Math.max(0, tw.cooldown), sx: onGlass ? (sp.x + 1) / 2 * innerWidth : null, sy: onGlass ? (1 - sp.y) / 2 * innerHeight : null }); }   /* the box is the seeker's cone at the camera's own field of view; the inner box closes on the held body's own point on the glass, and TARGET LOCKED ticks with the build cursor's cue */
  ...
  Object.assign(storyApi, { openBackDoor: () => { const m = story?.backMouth; if (!m || story.backOpen) return 0; story.backOpen = true; const down = [...m.cells, ...m.flank].filter((ci) => breachWallCell(ci)); rebuildAfterBreach(); recomputePortalDist(); for (const ci of down) explode('tank.shell', graph.centers[ci]); sfx.play('sinkhole_quake', { dist: camDist(m.dir) }); showBrief('back_door'); story.hud.back(m.dir); if (!paused && !deploy && !shotActive() && !pilotMode && !pilot?.gunship) startDiveShot({ camera, startShot, cellSide }, new THREE.Vector3(...m.dir), { id: 'backdoor', preRoll: story.backDoor.preRoll, hold: story.backDoor.hold, tail: story.breachShot.tail ?? 1.8, dive: story.backDoor.dive, fromCamera: true }); return down.length; },
    backDoorOpen: () => !!story?.backOpen, backBreachCandidates: () => (story?.backMouth ? backBreachCells(story.backPlanet, dungeon.tags, dungeon.heart, story.backMouth, story.backDoor).map(({ cell, hops }) => ({ cell, side: 'back', hops, route: dungeon.distToHeart[cell], pos: graph.centers[cell].slice() })) : []) });   /* hops: the route once the back is open; route: the live field now */
  ...
  const gameHooks = {
      ...
      begin: () => { endShot(); paused = false; }, setSector: (n) => { if (story) story.sectorN = n; }, breakGate: (id = 'gate') => !!sectorRun?.breakGate(id) /* the harness takes the door down: Isao's animated repair needs a broken door */, mountGunship: () => { if (!storyViews) storyApi.unlock('views'); if (automated() && !onStation(gunship)) storyViews.meter(callProgress(gunshipCall), callFull(gunshipCall)); else storyViews.station(onStation(gunship), phaseLeft(gunship)); document.querySelector('#story-views [data-mount="gunship"]')?.click(); return !!pilot?.gunship; }, gunshipHold: (on) => { if (pilot) pilot.state.held = !!on; }, gunshipCam: () => camera.quaternion.toArray(), programs: () => renderer.info.programs.map((p) => `${p.name}#${String(p.cacheKey).length}`) /* which shader programs are linked: the seat hitch probe */, gunshipGun: (k) => selectGun(gunship, k, GUNSHIP_GUNS), gunshipPassEnd: () => { gunship.left = 0; } /* the harness ends the station pass now: the next tick departs */, fillGunshipCall: (n) => fillFromKill(gunshipCall, n, GUNSHIP_CALL), spawnFodder: (n, type = 'amoeba') => { if (!story) return 0; if (!story.source?.alive) storyApi.breach(gunshipFar()); for (let i = 0; i < n; i++) storyApi.spawn(type, story.source.ci, { spread: 0.8, delay: i * 0.12 }); return n; },   /* the skip panel's enemies: a breach opens on the lane outside the gate if none is live, and they rise out of it staggered (emergence only runs from a real breach) */
      clearSector: () => {
        endShot(); 
      ...
      /* THE SHOWCASE (src/fx/showcase.js, src/content/showcase.js): the montage's rail drives the real systems through these and nothing else, so it needs no ?acceptance=1. `counters` is the cheap proof the --showcase step reads at every cut. */ showcase: { ready: () => { if (!story || !storyBase || !automated() || playerMesh?.userData.loading || deploy) return false; if (!story.source?.alive) { storyApi.breach(gunshipFar()); return false; }   /* THE PRE-ROLL IS THE LOAD: the montage's first frame waits here while the planet, the models and the base arrive, the breach opens and the first bodies climb out of it, so the rail never opens on an empty yard */ if (!story.showcaseSeeded) { story.showcaseSeeded = true; gameHooks.showcase.swarm(10); gameHooks.showcase.prep(); } return enemies.filter((e) => e.alive && e.id > 0 && e.spec.rammable && e.emergeAge >= 1.2).length >= 6; }   /* six is enough to ram: the world holds about seventy bodies at once and queues the rest, so a fat pre-roll starves the emergence shot */, begin: () => { endShot(); paused = false; params.callouts = false; }, counters: () => ({ rams: rs.rams, pilotRounds: rs.pilotRounds ?? 0, seatShots: pilot?.state?.shots ?? 0, mag: gunship.mag, enemies: enemies.filter((e) => e.alive).length, emerging: enemies.filter((e) => e.alive && !e.guard && e.emergeAge < 1.2).length, blasts: rs.bySrc.strike, explosions: Object.values(explosions.state().spawned).reduce((a, b) => a + b, 0), nuke: heavyState(gunship, GUNSHIP_GUNS).phase, nukeFlying: !!gunshipDrop?.mesh(), beamSeconds: laserStation.state().seconds, beamBurned: laserStation.state().burned.bodies, queued: spawnQueue.length, breaches: spawnPoints.filter((x) => x.alive && gameBreaches.ready(x.obj)).length, flight: towerSeekers.length, seekers: seekerHits + seekerLost + towerSeekers.length,   /* every guided round this run has left a rail: a 2.5 s shot cannot wait for one to land */ alive: enemies.filter((e) => e.alive && e.id > 0 && e.spec.rammable).length, laserOnline: laserStation.state().online, seat: pilot?.gunship ? 'gunship' : laserStation.seated() ? 'sol82' : pilot?.state?.tower?.key ?? null }), source: () => { if (!story) return -1; const sp = spawnPoints.find((x) => x.alive && gameBreaches.ready(x.obj)); if (sp) { story.source = sp; return sp.ci; }   /* A BREACH THE SWARM CAN USE: releaseSpawns holds the queue until the hole has finished opening, so the montage takes a breach that is already open (the sector's own) over a fresh one it would have to wait out */ if (!story.source?.alive) storyApi.breach(gunshipFar()); return story.source?.ci ?? -1; }, swarm: (n = 30, type = 'amoeba', gap = 0) =>   /* no stagger by default: a queued spawn waits on the spawn clock, which does not always run between waves, and a montage cannot wait for it */ { const ci = gameHooks.showcase.source(); if (ci < 0) return 0; for (let i = 0; i < n; i++) storyApi.spawn(type, ci, { spread: 0.9, delay: i * gap }); releaseSpawns(0); return n; }   /* THE MONTAGE RELEASES ITS OWN: the spawn clock only turns over inside the wave loop, so a queued swarm can sit there for a shot or two — this drains what is due now, through the same release path and the same breach-is-open gate */, tremor: () => storyApi.tremor(gameHooks.showcase.source()), orbit: (ci, dist = 1.5) => { endShot(); paused = false; setView('orbit'); followSuspend = true; if (ci >= 0) { buildQ.setFromUnitVectors(BQ_Z, new THREE.Vector3(...graph.centers[ci]).normalize()); buildDist = dist; } }, ground: (ci, height = 4, back = 4) => { if (ci < 0) return; endShot(); paused = false; startDiveShot({ camera, startShot, cellSide }, new THREE.Vector3(...norm3(graph.centers[ci])), { id: 'showcaseGround', hold: 600, fromCamera: true, dive: { height, back, diveSeconds: 0.35 } }); }, prep: () => { for (const key of ['quiver', 'rotor']) gameHooks.showcase.seat(key, false); leavePilot(); }, launch: () => { const tw = pilot?.state?.tower; if (!tw || !missileOf(tw.key)) return false; const e = enemies.find((x) => x.alive && x.id > 0) ?? { id: -1000, pos: graph.centers[gameHooks.showcase.source()] }; return !!launchTowerSeeker(tw, towerMuzzle(tw, graph.centers[tw.ci]), e, runContext.time); }, follow: () => { endShot(); paused = false; followSuspend = false; setView('third'); snapCamera(); },   /* the hull's own chase camera: a cinematic shot latches the camera AND holds the drive, and the ram is the hull moving */ ramNext: () => { const e = enemies.find((x) => x.alive && x.id > 0 && x.spec.rammable); if (!e) return false; gameHooks.placeTank(e.cur); return true; }, shell: () => fire(), gunship: () => { fillFromKill(gunshipCall, 99, GUNSHIP_CALL); if (!onStation(gunship)) startStation(gunship, GUNSHIP_ORBIT); if (!pilotMode) enterPilot(towers.map((tw) => tw.ci)); endShot(); return pilot?.mountGunship?.() === 'mounted'; }, gun: (k) => selectGun(gunship, k, GUNSHIP_GUNS), hold: (on = true) => { if (pilot) pilot.state.held = !!on; }, nuke: () => { const ci = enemies.find((e) => e.alive && e.id > 0)?.cur ?? gameHooks.showcase.source(); if (ci < 0) return false;   /* onto the swarm, not onto the hole: a strike on a breach cell SEALS it, and the montage still needs it open */ paintHeavy(gunship, ci, GUNSHIP_GUNS); return !!launchHeavy(gunship, GUNSHIP_GUNS); }, seat: (key, sit = true) => { let tw = towers.find((t) => t.key === key) ?? null; if (!tw) { const src = graph.centers[gameHooks.showcase.source()] ?? graph.centers[dungeon.spawn]; const ci = Object.keys(story?.socketToward ?? {}).map(Number).filter((c) => !towerByCell.has(c)).sort((a, b) => chord(graph.centers[a], src) - chord(graph.centers[b], src))[0]   /* the mount nearest the breach: a seat with nothing in reach is a seat with nothing to show */ ?? dungeon.tags.findIndex((tag, i) => !placeError(i) && !towerByCell.has(i)); if (ci === undefined || ci < 0) return -1; tw = commitTower(key, ci, 0); } if (!tw) return -1; if (!sit) return tw.ci; enterPilot([tw.ci, ...towers.map((t) => t.ci).filter((c) => c !== tw.ci)]); pilotHost?.pick?.(key); pilot?.select?.(key); endShot(); return tw.ci; }   /* pick alone leaves the post where the last seat left it, and the mount that opens is then whichever one stands there */, aim: () => { const tw = pilot?.state?.tower; if (!tw) return null; const reach = missileOf(tw.key)?.maxRange ?? effectiveStats(tw.def, tw.tier).range * 10, inReach = (x) => missileDistance(graph.centers[tw.ci], x.pos) < reach;   /* IN REACH FIRST: a mount pointed at a body it cannot hit holds its fire, and the shot is two seconds long */ const e = enemies.find((x) => x.alive && x.id > 0 && inReach(x) && losClear(tw.ci, x.pos, perchOf(tw))) ?? enemies.find((x) => x.alive && x.id > 0 && inReach(x)) ?? enemies.find((x) => x.alive && x.id > 0); const at = e ? e.pos : graph.centers[gameHooks.showcase.source()]; if (!at) return null; pilot.aimAt(add3(at, scale3(norm3(at), cellSide * 0.3))); return e?.id ?? 0; }, leave: () => { if (pilotMode) leavePilot(); }, laser: (on = true) => { laserStation.setOnline(on); if (!on) return false; laserStation.hooks.laserPassNow(); laserStation.hooks.laserSit(); laserStation.hooks.laserSteer('body'); return laserStation.seated(); }, laserHold: (on = true) => { if (!laserStation.seated()) return false; laserStation.hooks.laserSteer('ahead'); laserStation.hooks.laserHold(on); return true; }, laserCam: (on = true) => laserStation.hooks.laserCam(on), laserLeave: () => laserStation.hooks.laserSeat(false), isao: () => { if (!isao) spawnIsao();   /* he is only in the world while he has an order: the last card prints him first, then takes the story's own face-on closeup (storyApi.closeup) */ storyApi.closeup(); return !!isao; } },
    };
  ...
  function enterPilot(posts) { pilot?.dispose(); pilotMode = true;   // one optic at a time: a hand-over while already piloting replaces the panel. the story hands over its mounts
    ...
    if(urlParams.get('acceptance')==='1')window.__stalheartPilotTest={state:()=>({paused,seed:params.seed,points:params.points,sector:round,posts:pilotPosts.slice(),view:pilot.state.view,ci:pilot.state.tower.ci,key:pilot.state.tower.key,shots:pilot.state.shots,held:pilot.state.held,heat:pilot.state.tower.heat??0,overheated:!!pilot.state.tower.overheated,roundDmg:+(effectiveStats(pilot.state.tower.def,pilot.state.tower.tier).dmg*(story?.pilot.dmgMul??1)).toFixed(3),enemyHp:enemies.find(e=>e.alive&&e.id>0)?.spec.hp??null,wave,enemies:enemies.filter(e=>e.alive).length,tank:player.pos.slice(),camera:camera.position.toArray(),target:pilot.state.target?.id??null,ready:!pilot.state.tower.obj.userData.loading,aimError:pilot.state.tower.aimErr,lock:pilot.state.tower.lock,heart:heartHP}),select:key=>pilot.select(key),
      ...
      enemy:id=>{const e=enemies.find(e=>e.id===id);return e?{hp:e.hp,alive:e.alive}:null;},hold:on=>{pilot.state.held=!!on;},view:v=>pilot.setView(v), lock:()=>{const tw=pilot.state.tower;return {meter:tw.lock?.meter??0,locked:!!tw.lock?.locked,tgt:tw.pilotTarget&&!tw.pilotTarget.pilotAim?tw.pilotTarget.id:null,flight:towerSeekers.reduce((n,m)=>n+(m.by===tw?1:0),0)};}, aimSky:()=>{const c=graph.centers[pilot.state.tower.ci];pilot.aimAt(add3(c,scale3(norm3(c),cellSide*40)));}, reach:()=>{const tw=pilot.state.tower;return enemies.filter(e=>e.alive).map(e=>({id:e.id,type:e.type,m:+missileDistance(graph.centers[tw.ci],e.pos).toFixed(1),los:losClear(tw.ci,e.pos,perchOf(tw)),cell:e.cur,pos:e.pos.map(v=>+v.toFixed(5))}));}, round:()=>{const r=pilotHost?.round();if(!r)return null;camera.updateMatrixWorld();const v=new THREE.Vector3().fromArray(r.pos).project(camera);return {x:+v.x.toFixed(4),y:+v.y.toFixed(4),z:+v.z.toFixed(5),u:+r.u.toFixed(4),phase:r.phase,view:pilot.state.view,fov:+camera.fov.toFixed(2)};}   /* the round through the real camera, for the frame probe */
    };
  }
