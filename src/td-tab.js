...
export function initTdTab(root) {
  ...
  const gameHooks = {
      ...
      /* THE SHOWCASE (src/fx/showcase.js, src/content/showcase.js): the montage's rail drives the real systems through these and nothing else, so it needs no ?acceptance=1. `counters` is the cheap proof the --showcase step reads at every cut. */ showcase: { ready: () => !!story && !!storyBase && automated() && !playerMesh?.userData.loading, begin: () => { endShot(); paused = false; params.callouts = false; }, counters: () => ({ rams: rs.rams, pilotRounds: rs.pilotRounds ?? 0, seatShots: pilot?.state?.shots ?? 0, mag: gunship.mag, enemies: enemies.filter((e) => e.alive).length, emerging: enemies.filter((e) => e.alive && !e.guard && e.emergeAge < 1.2).length, blasts: rs.bySrc.strike, explosions: Object.values(explosions.state().spawned).reduce((a, b) => a + b, 0), nuke: heavyState(gunship, GUNSHIP_GUNS).phase, nukeFlying: !!gunshipDrop?.mesh(), beamSeconds: laserStation.state().seconds, beamBurned: laserStation.state().burned.bodies, laserOnline: laserStation.state().online, seat: pilot?.gunship ? 'gunship' : laserStation.seated() ? 'sol82' : pilot?.state?.tower?.key ?? null }), source: () => { if (!story) return -1; if (!story.source?.alive) storyApi.breach(gunshipFar()); return story.source.ci; }, swarm: (n = 30, type = 'amoeba') => { const ci = gameHooks.showcase.source(); if (ci < 0) return 0; for (let i = 0; i < n; i++) storyApi.spawn(type, ci, { spread: 0.9, delay: i * 0.05 }); return n; }, tremor: () => storyApi.tremor(gameHooks.showcase.source()), orbit: (ci, dist = 1.5) => { endShot(); paused = false; setView('orbit'); followSuspend = true; if (ci >= 0) { buildQ.setFromUnitVectors(BQ_Z, new THREE.Vector3(...graph.centers[ci]).normalize()); buildDist = dist; } }, ground: (ci, height = 4, back = 4) => { if (ci < 0) return; endShot(); paused = false; startDiveShot({ camera, startShot, cellSide }, new THREE.Vector3(...norm3(graph.centers[ci])), { id: 'showcaseGround', hold: 600, fromCamera: true, dive: { height, back, diveSeconds: 0.35 } }); }, ramNext: () => { const e = enemies.find((x) => x.alive && x.id > 0 && x.spec.rammable); if (!e) return false; gameHooks.placeTank(e.cur); return true; }, shell: () => fire(), gunship: () => { fillFromKill(gunshipCall, 99, GUNSHIP_CALL); if (!onStation(gunship)) startStation(gunship, GUNSHIP_ORBIT); if (!pilotMode) enterPilot(towers.map((tw) => tw.ci)); endShot(); return pilot?.mountGunship?.() === 'mounted'; }, gun: (k) => selectGun(gunship, k, GUNSHIP_GUNS), hold: (on = true) => { if (pilot) pilot.state.held = !!on; }, nuke: () => { const ci = gameHooks.showcase.source(); if (ci < 0) return false; paintHeavy(gunship, ci, GUNSHIP_GUNS); return !!launchHeavy(gunship, GUNSHIP_GUNS); }, seat: (key) => { let tw = towers.find((t) => t.key === key) ?? null; if (!tw) { const ci = Object.keys(story?.socketToward ?? {}).map(Number).find((c) => !towerByCell.has(c)) ?? dungeon.tags.findIndex((tag, i) => !placeError(i) && !towerByCell.has(i)); if (ci === undefined || ci < 0) return -1; tw = commitTower(key, ci, 0); } if (!tw) return -1; enterPilot([tw.ci, ...towers.map((t) => t.ci).filter((c) => c !== tw.ci)]); pilotHost?.pick?.(key); endShot(); return tw.ci; }, aim: () => { const tw = pilot?.state?.tower; if (!tw) return null; const e = enemies.find((x) => x.alive && x.id > 0 && losClear(tw.ci, x.pos, perchOf(tw))); if (!e) return null; pilot.aimAt(add3(e.pos, scale3(norm3(e.pos), cellSide * 0.3))); return e.id; }, leave: () => { if (pilotMode) leavePilot(); }, laser: (on = true) => { laserStation.setOnline(on); if (!on) return false; laserStation.hooks.laserPassNow(); laserStation.hooks.laserSeat(true); laserStation.hooks.laserSteer('body'); return laserStation.seated(); }, laserHold: (on = true) => { if (!laserStation.seated()) return false; laserStation.hooks.laserSteer('ahead'); laserStation.hooks.laserHold(on); return true; }, laserCam: (on = true) => laserStation.hooks.laserCam(on), isao: () => { storyApi.closeup(); return !!isao; } },
    };
