...
export function initTdTab(root) {
  ...
  if (urlParams.get('acceptance') === '1') {
    window.__stalheartTest = {
      ...
      begin: () => { endShot(); dismissIntro(); paused = false; tutorial.frozen = false; }, mountGunship: () => { if (!storyViews) storyApi.unlock('views'); storyViews.station(onStation(gunship), phaseLeft(gunship)); document.querySelector('#story-views [data-mount="gunship"]')?.click(); return !!pilot?.gunship; }, gunshipHold: (on) => { if (pilot) pilot.state.held = !!on; }, gunshipGun: (k) => selectGun(gunship, k, GUNSHIP_GUNS),
      clearSector: () => {
