...
export function initTdTab(root) {
  ...
  function frame(dt, simSkip) {
    ...
    if (!player.won && !frozen && !tutorialActive) {
      ...
      { const ev = stepGunship(gunship, dt, GUNSHIP_ORBIT); storyViews?.station(onStation(gunship), phaseLeft(gunship)); if (ev === 'arrive' && story) showBrief('gunship_pass'); if (ev === 'depart') gunshipWalls = null; pilot?.gunshipTick?.(dt); } if (stepStrike(strike, dt, strikeTune) === 'armed') {   // the gunship's pass, then the strike's ration
        sfx.play('tower_upgrade');
