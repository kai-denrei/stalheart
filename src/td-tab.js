...
export function initTdTab(root) {
  ...
  const strike = makeStrike(), gunship = makeGunship(GUNSHIP_ORBIT, { station: new URLSearchParams(location.search).get('gunship') === 'station' }); let gunshipOptic = null, gunshipWalls = null;   // the platform's schedule runs on the game clock beside the strike's ration; ?gunship=station opens its first pass at once, for acceptance
  ...
  if (urlParams.get('acceptance') === '1') {
    window.__stalheartTest = {
      state: () => ({ gunship: { phase: gunship.phase, left: +gunship.left.toFixed(2), mounted: gunship.mounted, gun: gunship.gun, passes: gunship.passes, optic: !!gunshipOptic?.active(), station: onStation(gunship), seat: !!pilot?.gunship }, shot:shotId(),breachRubble:gameBreaches.rubbleState(),breaches:gameBreaches.state(),queued:spawnQueue.length,wallCount:dungeon.tags.filter(tag=>tag===BLOCKED).length,emerging:enemies.filter(e=>e.alive&&e.emergeAge<1.2).length,round, wave, runGen: runContext.generation, heart: heartHP, hulls: playerHP,
