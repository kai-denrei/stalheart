...
import { STRIKE_KNOBS, makeStrike, makeStrikeParams, grantStrikes, stepStrike,
  ...
  strikeDamage, retargetStrike, orbitProgress } from './strike.js'; import { choosePilotPosts } from './domain/pilot-posts.js'; import { makeGunship, stepGunship, onStation, phaseLeft, passProgress, mountGunship, dismountGunship, selectGun, stepGun, aimOnSphere, splashDamage, dangerReport } from './domain/gunship.js'; import { GUNSHIP_GUNS, GUNSHIP_GUN_ORDER, GUNSHIP_PLATFORM, GUNSHIP_ORBIT } from './content/gunship.js'; import { createGunshipOptic } from './fx/gunship-optic.js'; import { createGunshipBriefing } from './fx/gunship-briefing.js'; import { createFoundryFx } from './fx/foundry-fx.js';
...
export function initTdTab(root) {
  ...
  const strike = makeStrike(), gunship = makeGunship(GUNSHIP_ORBIT, { station: new URLSearchParams(location.search).get('gunship') === 'station' }); let gunshipOptic = null, gunshipWalls = null, gunshipBriefing = null, foundryFx = null; const gunshipFar = () => { const d = bfsDist(graph.adj, [dungeon.heart], (ci) => dungeon.tags[ci] !== BLOCKED), want = Math.min(Math.round(60 * 1.15), d.reduce((m, v) => Math.max(m, v), 0)); let best = -1, bd = Infinity; for (let i = 0; i < d.length; i++) if (d[i] >= want - 3 && d[i] <= want + 3) { const c = chord(graph.centers[i], graph.centers[dungeon.spawn]); if (c < bd) { bd = c; best = i; } } return best >= 0 ? best : dungeon.spawn; }; /* a breach a minute's walk out (owner: leave time to enjoy the action): the open cell about 69 hops from the heart nearest the lane end, the other side of the map if the planet is smaller */ const gunshipLane = () => story?.source?.alive ? story.source.ci : story?.ring?.size ? [...story.ring][0] : dungeon.spawn;   // where they come from: the live breach, else the lane outside the gate, else the lane end. The platform's schedule runs on the game clock beside the strike's ration; ?gunship=station opens its first pass at once, for acceptance
    ...
    if (isao.obj.userData.setFace) {
      ...
      isao.obj.userData.setFace(
        isao.faceLock ?? (isao.gleeT > 0 ? 'glee'   // ONE HELD FACE PER STATE (owner, 2026-09-13: minimalism): glee when a print lands,
          : working ? 'determined'                  // determined while he builds, neutral otherwise. The crowd count and the
  ...
  function leavePilot() { if (!pilotMode) return; pilot?.dispose(); pilot = null; pilotHost = null; pilotMode = false; storyScope?.update({ on: false }); /* the scope leaves with the optic */ params.callouts = true; delete window.__stalheartPilotTest; setView('third'); snapCamera(); }   // back to the hull
