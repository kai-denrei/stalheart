import { InstancedBufferAttribute, InstancedMesh, Object3D, Quaternion, Vector3 } from '../../../../vendor/three.module.js';
...
export class MonolithRiftAbility extends Ability {
  ...
  onSpawn() {
    ...
    const wanted = Math.min(MAX_STONES, Math.max(0, Math.round(c.stoneCount * c.density)));
  ...
  _updateShrapnel(dt, retract) {
    ...
    for (let i = 0; i < this.chunks.length; i++) {
      ...
      let floorHeight=this.ctx.environment.groundHeightAt?.(chunk.position.x,chunk.position.z)??0;
      if(chunk.resting&&chunk.position.y>floorHeight+radius*.55+.01)chunk.resting=false;
      if (!chunk.resting && dt > 0) {
        chunk.velocity.y += gravity * dt;
        chunk.position.addScaledVector(chunk.velocity, dt);

        floorHeight=this.ctx.environment.groundHeightAt?.(chunk.position.x,chunk.position.z)??0;
        if (chunk.position.y <= floorHeight + radius * 0.55) {
          chunk.position.y = floorHeight + radius * 0.55;
  ...
  _impactPuff(chunk, c, g, impact) {
    _emit.position = _pos.copy(chunk.position).setY((this.ctx.environment.groundHeightAt?.(chunk.position.x,chunk.position.z)??0)+0.05);
    _emit.radius = 0.1;
