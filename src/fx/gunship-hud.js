...
export function createGunshipHud(root) {
  ...
  layer.innerHTML = `<svg class="reticle" aria-hidden="true"></svg><div class="ro"><div class="r-head split"><span>KORP / GS01</span><b data-f="gun">—</b></div>
    ...
    <div class="field"><span class="k">Fire</span><b class="v" data-f="state">—</b></div><div class="field"><span class="k">Mag</span><b class="v" data-f="mag">—</b></div></div>`;
  root.append(layer);
  const svg = layer.querySelector('svg'), f = {}; for (const b of layer.querySelectorAll('[data-f]')) f[b.dataset.f] = b;
  let last = '', spin = 0;
  ...
  function reticle(w, h, gun, hot, painted, spinDeg) {
    ...
    if (gun === 'rotary') {
      ...
      P.push(`<text x="${cx}" y="${cy + R * 1.5 + 12}" fill="${col}" ${mono} opacity="0.85">25MM · ROTARY${hot ? ' · FIRING' : ''}</text>`);
    } else if (gun === 'bofors') {
      ...
      P.push(`<text x="${cx}" y="${cy + b + 18}" fill="${col}" ${mono} opacity="0.85">40MM · BOFORS · AREA</text>`);
    } else {
      const d = R * 1.2, L = R * 3;
