...
export function createSentryPilot(root, host) {
  ...
  // GROUND TRUTH · IMPACT, framed per gun (owner, 2026-09-15: a bigger, more satisfying impact view): the long lens fits three blast radii and stands
  // back far enough to stay inside the monitor's 30° clamp, so a rotary pop, a Bofors burst and a 105's 64 m blast each fill the view at their own size
  const gunshipOptic=()=>{if(!gunship||!impact)return null;const c=host.cellSide(),span=Math.max(c*1.2,G.guns[G.state.gun].blastCells*c*3),back=Math.max(c*2.2,span*1.9);return{from:aim.fromArray(impact).addScaledVector(t1,back).toArray(),pos:impact,span,lift:back*0.45,label:'GROUND TRUTH · IMPACT'};};
