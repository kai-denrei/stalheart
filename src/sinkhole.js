...
const QUAKE_BASE=Object.freeze({...settings.quake});

// One casterless event, reused on retrigger. No monoliths, no distortion pass.
export function createSinkhole(scene,camera,{game=false,sounds=null}={}){
  ...
  const frameMatrix={value:new THREE.Matrix4()},inverseFrame={value:new THREE.Matrix4()};
  const surface=createBreachSurface(radius,frameMatrix,inverseFrame);
  const sound=sounds||makeAudio({sounds:BREACH_SOUNDS,persist:false});if(!sounds)sound.arm();
  ...
  const ctx={scene:group,camera,environment:{registerShadowCasterWithPatch:patchOnBeforeCompile,groundHeightAt:(x,z)=>sinkholeGroundHeight(x,z,hole.value)},
    particles:new ParticleEngine(group),lights:game?{acquire:()=>null,set(){},release(){},reset(){},update(){},dispose(){}}:new LightPool(group),decals:new DecalSystem(group),
  ...
  const terrain=makeSinkholeTerrain(hole,radius,{ground:!game});group.add(terrain.group);
  const enemies=game?{reset(){},update(){},dispose(){},state:()=>({spawned:0})}:createBreachEnemies(group);
  const walls=game?{update(){},dispose(){},state:()=>({total:0})}:createBreachWalls(group);
  ability.fissures.mesh.renderOrder=-20;
  ...
  return {group,tune,trigger,reset,rig,setSound(on){tune.sound=on;if(!on)sound.panic();},
    hole,frameMatrix,inverseFrame,
    ready:()=>getStoneTextures().state.loaded===4,
    update(dt,absoluteTime){if(disposed||!group.visible)return;
      radius.value=tune.environment==='planet'?tune.planetRadius:0;
      if(game){group.updateWorldMatrix(true,false);frameMatrix.value.copy(group.matrixWorld);inverseFrame.value.copy(frameMatrix.value).invert();}
      time=absoluteTime??(time+dt);frame.uTime.value=time;frame.uDelta.value=dt;frame.uCameraNear.value=camera.near;frame.uCameraFar.value=camera.far;
      const look=LOOKS[tune.look];
      if(!game)scene.background.set(look?.bg??0x04070d);terrain.setLook(tune.look);
      ...
      const openingDuration=Math.max(1,tune.duration-tune.preRoll);
      const animationRate=1.5/openingDuration;
      ...
      ctx.decals.group.traverse(o=>{if(o.isMesh)o.renderOrder=-21;});
      if(game)group.traverse(o=>o.layers.set(0));
      surface.wrap(group);
    ...
    dispose(){if(disposed)return;reset();disposed=true;ability.dispose();ctx.particles.dispose();ctx.decals.dispose();ctx.lights.dispose();
      terrain.dispose();walls.dispose();enemies.dispose();if(!sounds)sound.dispose();scene.remove(group);},
  };
}
