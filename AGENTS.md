...
- The story planet ships baked (`assets/story/planet.bin`). After changing `STORY_RECIPE` or the pinned kernel run `npm run bake`; `test/planet-bake.mjs` fails until the bake matches a fresh build.
- `npm run build` meshopt-packs every model into dist (sources untouched); `npm ci` first, gltfpack is a dev dependency. Landmarks with a `far` tier in `src/content/base-layout.js` are derived by `npm run tiers` and pinned in `docs/far-tier-assets.lock.json`; re-run it after changing a source model or `KIT.lod.ratio`.
