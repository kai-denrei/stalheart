...
Astro industrial imports use `labs/astro-batching.js` to preserve animated owners during static material merging, and `astro-foundations.js` instances authored tiles on one non-overlapping lattice. `domain/yard-drive.js` isolates flat-yard acceleration/braking/collision from main-game movement; `labs/astro-drive.js` owns mode-scoped input and chase-camera lifetime while reusing the game tank-feel adapter. No industrial power, logistics or construction gameplay is introduced.
