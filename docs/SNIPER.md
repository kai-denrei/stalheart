...
Manual training defaults to 5× shared maximum/minimum reach, selectable as Game 1×, Manual 3× or Manual 5×. Default target distance is 85% of that stage reach, capped at 150 m times the multiplier (at 5×: Lancer 298 m, Mortar 149 m, Needle 340 m), with 2× magnification and a 60 m zero. An explicit URL or edited distance remains under operator control. Lancer and Plasma start with three calibration plates and one 14-creature column, including shellback, prime and knot, using shared size/speed inputs and explicit live-fire target toughness (default 0.2× game HP; adjustable 0.05–1×); R resets the exercise. Calibration and crossing-contact modes remain selectable. The explicitly labelled Quiver/Javelin prototype enables 1,000 m acquisition (adjustable 100–2,000 m), one manual shot per lock, and a longer travel time when required. Damage is unchanged; automated Quiver retains the shared game limits. The explicit prototype reach remains an absolute override; other weapons use the selected stage multiplier. Inbound and crossing targets, breath, wind/gusts, gravity, drag, optic sway, magnification, calibration and optional assists remain this stage's simulation. Ballistic wind/drop and manual input are not a claim of full board-combat parity. Stage projectile speed is displayed read-only and scales by the square root of the reach multiplier so ballistic rounds can reach farther without changing gravity. Damage, cadence and shipped game values remain unchanged. Environment changes do not promote game defaults.
...
`src/labs/sniper-environment.js` uses the pinned sphere-grid kernel and shared TRON palette for a 2,600 m radius planet, with a prepared tangent firing terrace through a 1,400 m canyon. The surrounding surface curves away; aiming and target motion on the terrace retain local metre coordinates rather than spherical ballistics. Instanced walls and merged edges bound the canyon, with distant fortifications and six shared Quiver/Lancer models mounted on walls. The player optic sits on a six-metre wall platform. Planet overview shows the layout and returns to the optic before firing.

Walls stop beams, ballistic rounds and missiles. `src/domain/mortar-ground.js` solves the high arc from the actual raised muzzle against the same flight integrator, preserving launch-time wind. Range scaling lives in `src/domain/sniper-scale.js`; neither module changes the shared combat catalog. Tests cover immutable scaling, raised-muzzle ground solutions, six mounted assets, canyon cover, long-range mortar landing and unchanged Game 1× parity across all eight weapons.
