...
- Foundation v1: pure core/domain/content layers with dependency guards; shared immutable FX packages (base `stalheart-fx-6`); local per-subject review/apply/undo, project working copies, compact change summaries, JSON backups, explicit draft preview and deterministic promotion; impact and sound labs use the game's runtime builders/mixer.
...
- MÖRK intact hover tank is available for optional testing via `?unit=mork#units` in Workshop and `?creature=mork&cine=0#td` in game. Local pinned D0: 24,196 triangles / 50 merged batches; authored rig and nine-shell game rules preserved. Its cannon barrel now carries the existing red-hot shell cooldown gauge in game and Units, following recoil. MK-CX/2 remains default; device performance review is pending.
