...
## What landed on 2026-09-25 (the refactor round, branch `refactor` on `pacing`)
...
2. **The refactoring round continues** (owner, 2026-09-25): its robustness half has landed on branch `refactor`; extractions from the controller follow the survey's ranking (showcase hooks, the gunship rig, the first hull's host, the story api split); the untested URL-flag probes wait for the owner's yes.
