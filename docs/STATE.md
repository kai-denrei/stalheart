...
2. **Known gaps** (the 2026-09-16 entries): no second gate, so the back door stays open once cracked; Isao's gate repair is not animated; SOL-82 burns only the Stålheart among buildings; the gunship seat still costs ~100 ms the first time it is taken; the MK-9's ignition fireball reads oversized close up; one release per pass is Node-tested but not verified end to end in a browser; the laser scorch survives NEW RUN; Isao does not fly to the dropped crate; touch labels on the pad are untested; a print is a Y-scale rise without a clipping plane.
