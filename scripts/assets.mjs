...
for(const lockFile of ['docs/shell-assets.lock.json','docs/beam-audio.lock.json','docs/rotor-audio.lock.json','docs/laser-audio.lock.json','docs/rotor-fire-audio.lock.json','docs/gunship-rotary-audio.lock.json']){
 const lock=JSON.parse(readFileSync(resolve(root,lockFile),'utf8'));
 if(sha(readFileSync(resolve(root,lock.sourcePath)))!==lock.sourceSha256)throw Error(`Derived asset source changed: ${lockFile}`);
 for(const file of lock.files){
  if(!['assets/models/ordnance/olive-shell.glb','src/content/shell-geometry.js','assets/audio/sentry_beam_sustain.wav','assets/audio/rotor_spin.wav','assets/audio/laser_burn_ground.wav','assets/audio/rotor_pov_fire_loop.wav','assets/audio/gunship_rotary_fire_loop.wav'].includes(file.path))throw Error('Unexpected derived asset path');
