...
try{
 ...
 {assert.equal(await evaluate('window.__stalheartTest.mountGunship()'),false,'the first seat opens the briefing, not the guns');assert(await evaluate('window.__stalheartTest.state().gunship.briefing'),'the briefing is open');assert(await evaluate('window.__stalheartTest.state().paused'),'the game waits under it');
  ...
  await send('Input.dispatchKeyEvent',{type:'keyDown',key:'m',code:'KeyM'});await send('Input.dispatchKeyEvent',{type:'keyUp',key:'m',code:'KeyM'});await delay(400);assert(await evaluate('document.querySelector("#tab-td").classList.contains("gunship-thermal")'),'M cycles to thermal');assert.equal(await evaluate('getComputedStyle(document.querySelector("#td-app canvas")).filter'),'url("#flir")','thermal is the FLIR ironbow');current='gunship-thermal';await finish();
 ...
 assert.equal(await evaluate('document.querySelectorAll("#story-views [data-mount]:not([data-mount=gunship])").length'),2,'one button per mount (the gunship has its own)');
