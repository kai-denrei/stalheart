...
const base = arg('--base', '/');
const authoring = !args.includes('--read-only') && existsSync(resolve(root, 'scripts/presets.mjs')) && existsSync(resolve(root, 'src/content/shipped.js'))
  ? createAuthoringService(root) : null;
...
const server = createServer(async (req,res) => {
  try {
    ...
    if (authoring && await authoring(req, res, route)) return;
    if (route === '__authoring/state' && req.method === 'GET') {
